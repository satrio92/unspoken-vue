<template>
    <div id="post-widget" class="flex flex-col items-center gap-5 bg-black-gradient rounded-[12px] xl:rounded-[20px] py-7 w-full text-white">
        <h4 class="text-[21px] xl:text-[28px] font-bold">{{ title }}</h4>
        <div class="flex flex-col">
            <router-link :to="'/post/' + item.title.replace(/[\s:;*?<>]+/g, '-').toLowerCase()" class="flex items-center gap-3 py-4 border-t border-t-white" v-for="item in myPosts.slice(-3)" :key="item.id">
                <img :src="'/assets/posts/' + item.title.replace(/[\s:;*?<>]+/g, '-').toLowerCase() + '/img/header.jpg'" :alt="item.title" class="w-[34px] xl:w-[40px] h-[34px] xl:h-[40px] rounded-full bg-pink-300">
                <div class="flex flex-col w-[200px] xl:w-[220px] gap-1">

			...
			<div class="w-full py-6 absolute bottom-0 left-0 flex flex-col items-center gap-2 z-20 transition-all duration-300 ease-in">
				...
				<div class="flex items-center gap-6 text-xs text-white ">
					...
					<div class="flex items-center gap-2">
						<img :src="'/assets/img/author/' + featuredPost[currentPost].author.name.replace(/[\s:;*?<>]+/g, '-').toLowerCase() + '.png'" :alt="featuredPost[currentPost].author.name" class="w-6 h-6 rounded-full bg-white">
						<p>{{ featuredPost[currentPost].author.name }}</p>
					</div>
				</div>
				<router-link :to="'/post/' + featuredPost[currentPost].title.replace(/[\s:;*?<>]+/g, '-').toLowerCase()" class="w-[240px] h-14 bg-dropsunrise rounded-[10px] mt-2 justify-center items-center text-lg text-black font-semibold hidden group-hover:flex">Read More</router-link>
			</div>
		</div>
	</div>
</template>
